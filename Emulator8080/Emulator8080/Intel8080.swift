...
class Intel8080: NSObject {
    ...
    public struct Registers {
        ...
        // Note: See the Intel8080.registerM variable
        var M: UInt8 = 0x00
    ...
    public var registerM: UInt8 {
        ...
        get {
            ...
            registers.M = value // HACK: This is a hack to allow KeyPath-based ops
            return value
        } set {
            memory[Int(registers.HL)] = newValue
            registers.M = newValue // HACK: This is a hack to allow KeyPath-based ops
        }
    }
    ...
    private func updateZSPACFlags(for result: UInt8, updateAuxiliaryCarry: Bool = true) {
        ...
        // Parity flag
        let onesCount: Int = bits.filter { $0 == true }.count
        if (onesCount % 2 == 0) {
            flags.parity = true
        } else {
            flags.parity = false
    ...
    private func registerKeyPathForThreeBitCode(_ code: UInt8) -> WritableKeyPath<Registers, UInt8> {
        switch (code) {
        case 0b000:
            return \Registers.B
        case 0b001:
    ...
    private func printState(disassemble: Bool = true) {
    ...
    func execute() {
        ...
        let codeIndices = memory.indices.filter { $0 >= programCounter }
        let code = codeIndices.map { memory[$0] }
        ...
        default:
            if (0x40...0x7f).contains(instruction) { // MOV
                ...
                registers[keyPath: dstKeyPath] = registers[keyPath: srcKeyPath]
                ...
                switch(op) {
                case 0b000: // ADD
                    let result = UInt16(registers.A) &+ UInt16(registers[keyPath: regKeyPath])
                    updateCarryFlag(for: result)
                    registers.A = getLowerByte(of: result)
                    updateZSPACFlags(for: registers.A)
                    break
                case 0b001: // ADC
                    let result = UInt16(registers.A) &+ UInt16(registers[keyPath: regKeyPath]) &+ (flags.carry ? 1 : 0)
                    updateCarryFlag(for: result)
                    registers.A = getLowerByte(of: result)
                    updateZSPACFlags(for: registers.A)
                    break
                ...
                case 0b011: // SBB
                    ...
                    break
                case 0b100: // ANA
                    registers.A = registers.A & registers[keyPath: regKeyPath]
